 ...
 * @author mespinoza
 */
@ManagedBean(name = "cursoParaleloMB")
@ViewScoped
public class CursoParaleloMB implements Serializable{
    ...
    private List<DocenteList> docenteList;
    ...
    { 
        ...
        docenteList         = servicioAplicacion.consultaDocenteCurso( "", "", "" );
    }
    
    public void showCursoParalelo(ActionEvent ae) {
        System.out.print("cursoParaleloMB Ejecuta showCursoParalelo");
      RequestContext.getCurrentInstance().openDialog("pag_asignacursod", getDialogOptions(), null);
  }
    ...
    */
        ...
        if( (identificacion   == null) || (identificacion.equals(""))   || 
          ...
          else
          {
              if (cedulaValida){
                ...
                int cantidad_docenteCurso = 0;
                int cantidad = 0;
                Long docente_id = Long.valueOf("0");
                
                cantidad_docenteCurso  = docenteList.size();
                
                System.out.println("cantidad de Docentes Asignados : "+cantidad);
                
                for (int i = 0; i < docenteList.size(); i++) 
                {
                  Long curso = docenteList.get(i).getCursoparaleloid();
                  docente_id = docenteList.get(i).getDocente_id();
                  System.out.println("Curso asignado al docente : "+curso);
                ...
                if( cantidad_docenteCurso != 0 && cantidad != 0)
                {
                    //Si existe Docente con el curso Seleccionado registrado.-
                ...
                {
                    if(cantidad_docenteCurso == 0){
                        //Si no existe Docente Registrado, lo registra.-
                        System.out.println("Invoca Metodo Guardar Docente: ");
                        
                        //Crea Usuario.-
                        TblUsuarios user = creaUsuario();
                        
                        docente_id = guardaDocente( user, identificacion, nombres, apellidos, fechaNac, genero,
                                                    correo, Estado);
                        
                        //Asigna Curso al Docente Registrado.-
                        asignarCursoDocente( docente_id, cursoparalelo_cod);
                        
                        
                        
                        context.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO,"Registro de Docente y curso asignado con éxito.", "Registro de Docente y curso asignado con éxito." ) );
                        ...
                        //Asigna Curso al Docente Registrado.-
                        System.out.println("ingresaCursoParalelo docente_id: "+docente_id);
                        asignarCursoDocente(docente_id, cursoparalelo_cod);
                        context.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO,"Curso asignado al docente con éxito.", "Curso asignado al docente con éxito." ) );
                    }
                }           
    ...
    // METODO Busca Cursos Paralelos Asignados al Docente .-
    public void buscaCursosParalelosAsignados() 
    {
        ...
        docenteList = servicioAplicacion.consultaDocenteCurso( identificacion, nombres, apellidos );
    }
    
    // METODO Busca Cursos Paralelos Asignados .-
    public void verDocentes() 
    {
        buscaCursosParalelosAsignados();
        System.out.println("verDocentes identificacion: "+identificacion+
                            " Cant Reg: "+docenteList.size());
        for (int i = 0; i < docenteList.size(); i++) {
            System.out.println("verDocentes identificacion: "+identificacion);
            System.out.println("verDocentes getIdentificacion: "+docenteList.get(i).getIdentificacion());
            if(docenteList.get(i).getIdentificacion().equals(identificacion) )
            {
                System.out.println("verDocentes Nombre: "+docenteList.get(i).getNombres());
                setNombres(docenteList.get(i).getNombres()); 
                setApellidos(docenteList.get(i).getApellidos());
                setFechaNac(docenteList.get(i).getFecha_nacimiento());
                setGenero(docenteList.get(i).getGenero());
                setCorreo(docenteList.get(i).getCorreo());
    ...
    */
    public Long guardaDocente( TblUsuarios usuarios, String identificacion1, String nombres1, String apellidos1, Date fechaNac, String genero1,
                                    String correo1, String estado1)
    {
        ...
        System.out.println("guardaCursoParalelo Usuario: "+usuarios.getId()+
                            " nombres: "+nombres1+
    ...
    /*
        METODO ASIGNA CURSO PARALELO AL DOCENTE REGISTRADO.-
    */
    public void asignarCursoDocente( Long docente_id1, Long cursoparalelo_cod1)
    {
        TblDocentecurso docentecurso = new TblDocentecurso();
        fechaActual         =   new Date();
        IdUserSession       =   getUserSessionId();
        
         System.out.println("asignarCursoDocente docente_id: "+docente_id1+
                " cursoparalelo_cod:"+cursoparalelo_cod1);
        
        TblDocente docente = new TblDocente();
        docente.setDocenteId(docente_id1);
        ...
        docentecurso.setDocentecursoSts("A");
        docentecurso.setDocentecursoUpdusr(" ");
        docentecurso.setDocentecursoUpdtim(fechaActual);
        docentecurso.setDocentecursoDltusr(" ");
        docentecurso.setDocentecursoDlttim(fechaActual);
        
        servicioAplicacion.ingresaDocenteCurso(docentecurso);
        
    }    
    
    public void resstableceDocente()
    {
        identificacion  = "";
        nombres         = "";
        apellidos       = "";
    ...
    public TblUsuarios creaUsuario()
    {
        ...
        System.out.println("apellidos  en CursoParaleloMB:"+apellidos);
        System.out.println("inicialUser  en creaUsuario:"+inicialUser);
        int pos     = apellidos.length();
        ...
        try 
        {
            ...
            System.out.println("Id de Usuario en creaUsuario CursoParaleloMB :" +idUsr);
            usuario.setId(idUsr);
            
            rol_cod  =   Long.valueOf("2");
    ...
    public List<DocenteList> getDocenteList() {
        return docenteList;
    }

    public void setDocenteList(List<DocenteList> docenteList) {
